...
{  GPIO_InitTypeDef GPIO_InitStructure;
   ...
   GPIO_InitStructure.GPIO_Pin =GPIO_Pin_0|GPIO_Pin_1|GPIO_Pin_2|GPIO_Pin_4|GPIO_Pin_5;	//打开C口的第几脚
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;//推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//速度50m
  GPIO_Init(GPIOC, &GPIO_InitStructure);
	LCD_PS_0; //12864的第15脚，为0是串行驱动，为1是并行驱动
	delay_us(10000);
  LCD_RST_0;//对液晶进行复位
	delay_us(10000);
 LCD_RST_1;
